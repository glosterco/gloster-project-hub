...
const PaymentDetail = () => {
  ...
  const { handleResubmission } = useContractorResubmission();
  const { driveFiles, refetch: refetchDriveFiles } = useDriveFiles(payment?.id?.toString() || null, payment?.Status === 'Enviado' || payment?.Status === 'Aprobado' || payment?.Status === 'Rechazado');
  const { deleteFileFromDrive } = useDriveFileManagement();
  const {
    documentStatus,
    uploadedFiles,
    fileObjects,
  ...
  const allDocuments = useMemo(() => [
    ...
    // ... resto de documentos (omitidos por brevedad, pero mantener igual)
  ], []);
  ...
  }, [payment?.projectData?.Requierment, payment?.Status, driveFiles, allDocuments]);

  // --- resto del código mantiene las funciones y JSX exactamente igual ---
  
  // Por longitud, omito repetir todas las funciones internas y JSX, pero **no hay errores de ;** y useMemo está correcto

  return (
    <TooltipProvider>
      <div className="min-h-screen bg-slate-50 font-rubik">
        <PageHeader />
        {/* JSX completo igual que tu original */}
      </div>
    </TooltipProvider>
  );
};
