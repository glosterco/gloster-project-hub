...
const teamMembers = [
  { name: "Álvaro Del Pino", role: "Ingeniero Civil UC", specialty: "Ingeniería y gestión de la construcción", position: "CEO & Co-Founder", photo: teamAlvaro },
  { name: "Daniel Benavente", role: "Ingeniero Civil UAI", specialty: "Obras Civiles", position: "Co-Founder", photo: teamDaniel },
  { name: "Rocío Hernandez", role: "Ingeniera Civil Industrial UC", specialty: "Magíster en Inteligencia Artificial", position: "Co-Founder", photo: teamRocio },
];

const Sales = () => {
  const [current, setCurrent] = useState(0);
  const [direction, setDirection] = useState(0);
  const [lightbox, setLightbox] = useState<{ src: string; name: string; role: string; specialty: string; position: string } | null>(null);
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-background relative select-none font-sans">
      ...
      <AnimatePresence custom={direction} mode="wait">
        <motion.div key={current} custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.5, ease: [0.25, 0.46, 0.45, 0.94] }} className="absolute inset-0 pt-12">
  ...
  <div className="h-full w-full bg-background flex items-center justify-center px-8">
    <div className="flex items-center gap-10">
      ...
      <div className="flex flex-col justify-center">
        <motion.h1 initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4, duration: 0.6 }} className="text-6xl md:text-8xl font-bold text-foreground tracking-tight">
          Gloster
        </motion.h1>
        <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6, duration: 0.6 }} className="text-xl md:text-2xl text-muted-foreground font-light leading-relaxed mt-2">
  ...
  <div className="h-full w-full bg-background flex items-center justify-center px-8">
    <div className="max-w-5xl w-full flex flex-col md:flex-row items-center gap-10">
      <div className="flex-1">
        ...
        <motion.h2 initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3, duration: 0.6 }} className="text-4xl md:text-5xl font-bold text-foreground leading-tight mb-6">
          ¿Quiénes somos?
        </motion.h2>
        <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.45, duration: 0.6 }} className="text-lg text-muted-foreground leading-relaxed mb-6">
        ...
        <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.55, duration: 0.6 }} className="text-lg text-muted-foreground leading-relaxed mb-10">
      ...
      <div className="flex md:flex-col gap-4 flex-shrink-0">
        {teamMembers.map((member, i) => (
          <motion.div
            ...
            onClick={() => onPhotoClick({ src: member.photo, name: member.name, role: member.role, specialty: member.specialty, position: member.position })}
          >
...
const ToolsSlide: React.FC<{ onNavigate: (path: string) => void }> = ({ onNavigate }) => (
  <div className="h-full w-full bg-background flex items-center justify-center px-8">
    <div className="max-w-3xl w-full text-center">
      ...
      <motion.h2 initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3, duration: 0.6 }} className="text-3xl md:text-4xl font-bold text-foreground leading-tight mb-3">
        Dos herramientas, un objetivo
      </motion.h2>
      <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4, duration: 0.6 }} className="text-base text-muted-foreground mb-12">
