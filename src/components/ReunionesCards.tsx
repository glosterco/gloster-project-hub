...
interface ReunionesCardsProps {
  reuniones: Reunion[];
  loading: boolean;
  onCardClick?: (reunion: Reunion) => void;
}

export const ReunionesCards: React.FC<ReunionesCardsProps> = ({
  reuniones,
  loading,
  onCardClick
}) => {
  if (loading) {
    return (
      <div className="space-y-6">
        <div>
          <h3 className="text-2xl font-bold text-slate-800 mb-6 font-rubik">Reuniones del Proyecto</h3>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="space-y-2">
                <h3 className="font-semibold text-slate-800 font-rubik">No hay reuniones registradas</h3>
                <p className="text-muted-foreground text-sm font-rubik">
                  Las reuniones aparecerán aquí cuando sean creadas
                </p>
              </div>
            </div>
          </CardContent>
